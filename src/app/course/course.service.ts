...
export class CourseService {
    ...
    async getAll() {
        return await this.courseModel.find().populate('videos', null, 'Video').populate('users', "-password", 'User').exec();
    }

    async getById(id: String) {
        return await this.courseModel.findById(id).populate('videos', null, 'Video').populate('users', "-password", 'User').exec();
    }
